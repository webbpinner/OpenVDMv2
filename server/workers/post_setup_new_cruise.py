...
customTaskLookup = [
    {
        "taskID": "0",
        "name": "postSetupNewCruise",
        "longName": "Post Setup New Cruise",
    }
]

commandFile = '/usr/local/etc/openvdm/postSetupNewCruise.yaml'
...
def task_postSetupNewCruise(worker, job):
...
def main(argv):
    ...
    parser = argparse.ArgumentParser(description='Handle post setup new cruise tasks')
    ...
    new_worker.set_client_id('postSetupNewCruise.py')

    debugPrint('Registering worker tasks...')
    debugPrint('   Task:', 'postSetupNewCruise')
