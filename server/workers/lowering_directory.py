...
def build_directorylist(gearman_worker):
    ...
    returnDirectories = [ loweringDir ]

    collectionSystemTransfers = gearman_worker.OVDM.getActiveCollectionSystemTransfers(cruise=False)
    returnDirectories.extend([ os.path.join(loweringDir, build_destDir(gearman_worker, collectionSystemTransfer['destDir'])) for collectionSystemTransfer in collectionSystemTransfers ])
...
def task_createLoweringDirectory(gearman_worker, gearman_job):
    ...
    if os.path.exists(loweringDataBaseDir):
        ...
        debug.error("Lowering Data Directory doesn not exist: {}".format(loweringDataBaseDir))
    ...
    if not os.path.exists(loweringDir):
        ...
        debug.error("Lowering directory already exists: {}".format(loweringDir))
    ...
    output_results = set_ownerGroupPermissions(warehouseUser, loweringDir)
...
def task_setLoweringDataDirectoryPermissions(gearman_worker, gearman_job):
    ...
    if os.path.isdir(loweringDir):
        ...
        set_ownerGroupPermissions(warehouseUser, loweringDir)
...
def task_rebuildLoweringDirectory(gearman_worker, gearman_job):
