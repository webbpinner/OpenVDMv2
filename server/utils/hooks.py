...
def get_post_hook_commands(gearman_worker, hook_name):
    ...
    try:
        ...
        if hook_name == POST_COLLECTION_SYSTEM_TRANSFER_HOOK_NAME:
            ...
            commands_from_file = list(filter(lambda collectionSystem: collectionSystem['collectionSystemTransferName'] == gearman_worker.collectionSystemTransfer['name'], commands_from_file))
            ...
            logging.warning("Invalid hook name: '%s' specified", hook_name)

    except Exception as err:
        logging.error(str(err))
        return {"verdict": False, "reason": "Could not process command file: " + DEFAULT_CONFIG_FILE, "commandList": []}

    return_command_list = build_commands(gearman_worker, commands_from_file)

    return {"verdict": True, "commandList": return_command_list}
